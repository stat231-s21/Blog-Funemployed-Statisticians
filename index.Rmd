...
#Isolates the state name and discards information regarding region within state
  separate(col = RegionName, into = c("RegionName2", "extra"), sep = ":", remove = FALSE)

#Combines "blank map" with data on housing
FullMap <- usa_states_leaflet2 %>%
  left_join(MapTable, by= c("RegionName2" = "RegionName"))
...
leaflet(data = usa_states_leaflet) %>%
  ...
  addPolygons(fillColor = ~mypal(FullMap$price)
              , color = "#FFFF33"  #color needs to be in hex format
              , fillOpacity = 0.7
              , stroke = FALSE
